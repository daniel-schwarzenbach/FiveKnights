...
TimePoint get_TimePoint() { return std::chrono::system_clock::now(); }
...
double Stopwatch::time() {
   ...
   if (endTime <=
       startTime) {  // This handles uninitialized or improperly reset `endTime`
...
void Stopwatch::stop() { endTime = std::chrono::system_clock::now(); }

// Sleep for x seconds
void sleep(double x) {
   auto duration = std::chrono::duration<double>(
       x);  // Creates a duration in seconds (double type)
