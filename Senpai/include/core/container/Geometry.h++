...
struct Rectangle {
   ...
   Vec2<T> center() const { return position; }

   // get normal vectors in case of a collision
   static Tuple<Vec2<T>, Vec2<T>> get_normal_vectors(Rectangle const& r1,
                                                     Rectangle const& r2) {
...
struct Frame {
   ...
   void add_offset(Vec2<T> const& offset) { position += offset; }
   ...
   Frame apply_parrent_frame(Frame const& parent) const {
      ...
      if (abs(parent.rotation - 180) < ε) {
         sinA = 0;
         cosA = -1;
      } else if (abs(parent.rotation - 90) < ε ||
                 abs(parent.rotation - 270) < ε) {
      ...
      result.position = {(relativePos.x * cosA - relativePos.y * sinA),
   ...
   Frame project(Rectangle<T> const& Camera, Rectangle<T> const& Canvas) const {
      ...
      return Frame{.position = (position - Camera.position) * scalefactor +
                               Canvas.position,
